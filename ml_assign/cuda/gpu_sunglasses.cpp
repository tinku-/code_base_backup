...
__global__ void cudakernel1(double *buf)
{
...
void forward_prop_inputlayer()
{
  ...
  cudaMemcpy(g_wh,wh,H*I*sizeof(double),cudaMemcpyHostToDevice);
  cudaMemcpy(g_input,input,I*sizeof(double),cudaMemcpyHostToDevice);
  
  cudakernel1<<<H/256, 256>>>(d_data);
  
  cudaMemcpy(wh,g_wh,H*I*sizeof(double), cudaMemcpyDeviceToHost);
