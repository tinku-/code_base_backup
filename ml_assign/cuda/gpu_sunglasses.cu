...
__global__ void cudakernel(double *bg_wh,double *bg_input,double *bg_hidden)
{
   double temp = 0.0;
   int i = threadIdx.x + blockIdx.x * blockDim.x;
   
   for(int j=0;j<I;j++)
		temp += bg_wh[i*H+j]*bg_input[j];
   bg_hidden[i] = (1.0/(1.0 + exp(-1*temp)));
}

void forward_prop_inputlayer()
{
  double whh[H*I];
  double *g_wh;	
  double *g_input;
  ...
  cudaMalloc(&g_wh, H*I*sizeof(double));
  cudaMalloc(&g_input, I*sizeof(double));
  cudaMalloc(&g_hidden, H*sizeof(double));

  cudaMemcpy(g_wh,whh,H*I*sizeof(double),cudaMemcpyHostToDevice);
  cudaMemcpy(g_input,input,I*sizeof(double),cudaMemcpyHostToDevice);
  cudaMemcpy(g_hidden,hidden,H*sizeof(double),cudaMemcpyHostToDevice);
  
  cudakernel<<<H/256, 256>>>(g_wh,g_input,g_hidden);
  
  cudaMemcpy(whh,g_wh,H*I*sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(input,g_input,I*sizeof(double), cudaMemcpyDeviceToHost);
  cudaMemcpy(hidden,g_hidden,H*sizeof(double), cudaMemcpyDeviceToHost);
