...
Author Names :	Bhavya Mishra
				...
				Ridam Jain
Dated : 9/11/2016
ID3 algorithm implementation to learn machine using decision tree
...
4. Build tree
	...
	c. if not , calculate the max entropy gain attribute
		a1.separate the input as per the attribute values
		a2. go to step four 	
5. process test data, and encode it similar to training data 
6. predict the output and compare it with the training data 
...
void predict(vector<int > v,treenode * root , int ans)//to calculate the accuracy 
{
	while(root->flag==0 )
	{
...
void resolve_op(treenode * root,string s)//to resolve the output , similar to the resolve_name function
{
	int len =s.size();
	int j = 0 ;
	int i = 0; 
...
void test(treenode * root)//to test the input data by calling predict function for each line int he file , skips the first line as the input data doesnt contain the first line 
{
	string s ; 
	ifstream f2;
...
int main() // main function 
{
	...
	treenode * root =new treenode;//new root node; 
	root = ID3(indexes,used_attr,13);	// ID3 function called 

	cout << "tree created " << endl ; //creation of tree done 
	
	test(root); //testing the inputs 
