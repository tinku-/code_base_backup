...
7. process test data, and encode it similar to training data 
8. predict the output using all the trees created and compare it with the training data
9. answer the one which comes in majority and compare it with the test data  
...
#define pb push_back 
#define NOT 350 // Number of trees created in the forest
...
treenode * ID3(vector<int > indexes , vector<int > used_attr , int depth)//create the tree ,recursively by taking 3 vectors in place , indexes to take care of what is the set of values that are needed to be considered among the whole dataset , used_attr contains the array of 0s and 1s where 1 is in the index of attribute that is already used and 0 otherwise. depth is the depth of the tree
{
	...
	set<int> next_attr; //contains attributes index to be considered in the next split
	//srand(time(0));
	srand(rand());
	...
	else
	{
		...
		while(next_attr.size()!=NOA)//generates 5 unique attributes that are not used before
		{
			r=rand()%14;
			if(used_attr[r]==0)
			{
...
void predict(vector<int > v, int ans)//to calculate the accuracy 
{
	...
	//if more locally expected output equals the answer, correct output is oberved
...
void test()//to test the input data by calling predict function for each line int he file , skips the first line as the input data doesnt contain the first line 
{
	...
	f2.open(testfile, ifstream::in);//testing file	
	  int i = 0 ; 
...
void clear_resources()//to clear all the memory used
{
...
int main(void)
{
	...
	clear_resources();	//free all the memory used
